...
MACRO(LINK_3RD_LIBS)
	foreach(ext_lib ${TARGET_3RD_LIBS})
	  STRING(TOUPPER "${ext_lib}" ext_lib_upper)
	  target_link_libraries( "${TARGET_NAME}" optimized "${${ext_lib_upper}_LIBRARY}" debug "${${ext_lib_upper}_LIBRARY_DEBUG}")	
	endforeach()
ENDMACRO(LINK_3RD_LIBS)
...
MACRO(SETUP_JS_PLUGIN PLUGIN_NAME)
    SET(CMAKE_LIBRARY_OUTPUT_DIRECTORY_DEBUG ${PROJECT_BINARY_DIR}/bin/Debug/js)
    SET(CMAKE_LIBRARY_OUTPUT_DIRECTORY_RELEASE ${PROJECT_BINARY_DIR}/bin/Release/js)
    SET(TARGET_NAME "${PLUGIN_NAME}_js" )
    SET(TARGET_TARGETNAME "${TARGET_DEFAULT_PREFIX}${TARGET_NAME}")
    ADD_LIBRARY(${TARGET_TARGETNAME} MODULE ${TARGET_SRC} ${TARGET_H})
    SET_TARGET_PROPERTIES(${TARGET_TARGETNAME} PROPERTIES PROJECT_LABEL "${TARGET_LABEL}")
    SET_TARGET_PROPERTIES(${TARGET_TARGETNAME} PROPERTIES FOLDER "Interp")
    IF(TARGET_LIBRARIES)
        LINK_INTERNAL(${TARGET_NAME} ${TARGET_LIBRARIES})
    ENDIF()
ENDMACRO(SETUP_JS_PLUGIN)
...
MACRO(SETUP_APPLICATION APPLICATION_NAME)
        ...
        SET_TARGET_PROPERTIES(${TARGET_TARGETNAME} PROPERTIES FOLDER "App")
