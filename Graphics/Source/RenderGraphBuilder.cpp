#include "Graphics/RenderGraphBuilder.h"
#include "Graphics/ForwardRenderPass.h"
#include "Graphics/FrustumCullPass.h"
#include "Graphics/Swapchain.h"
#include "Graphics/DescriptorManager.h"
#include "Platforms/IWindowWidget.h"
#include <stdexcept>

namespace xcel {

struct RenderGraphBuilder::Impl
{
    PassOptions        options;
    Swapchain*         swapchain    = nullptr;
    VkSurfaceKHR       surface      = VK_NULL_HANDLE;
    IWindowWidget*     window       = nullptr;
    DescriptorManager* descriptors  = nullptr;
    std::string        shaderDir    = "shaders/";
    uint32_t           maxObjects   = 65536;
};

RenderGraphBuilder::RenderGraphBuilder()
    : m_impl(std::make_unique<Impl>()) {}

RenderGraphBuilder::~RenderGraphBuilder() = default;

RenderGraphBuilder& RenderGraphBuilder::SetOptions(const PassOptions& opts)
{
    m_impl->options = opts;
    return *this;
}

RenderGraphBuilder& RenderGraphBuilder::SetSwapchain(Swapchain& swapchain)
{
    m_impl->swapchain = &swapchain;
    return *this;
}

RenderGraphBuilder& RenderGraphBuilder::SetSurface(VkSurfaceKHR surface)
{
    m_impl->surface = surface;
    return *this;
}

RenderGraphBuilder& RenderGraphBuilder::SetWindow(IWindowWidget& window)
{
    m_impl->window = &window;
    return *this;
}

RenderGraphBuilder& RenderGraphBuilder::SetDescriptors(DescriptorManager& descriptors)
{
    m_impl->descriptors = &descriptors;
    return *this;
}

RenderGraphBuilder& RenderGraphBuilder::SetShaderDir(const std::string& dir)
{
    m_impl->shaderDir = dir;
    return *this;
}

RenderGraphBuilder& RenderGraphBuilder::SetMaxObjects(uint32_t maxObjects)
{
    m_impl->maxObjects = maxObjects;
    return *this;
}

RenderGraph RenderGraphBuilder::Build(DeviceContext& dev)
{
    if (!m_impl->swapchain)   throw std::runtime_error("RenderGraphBuilder: swapchain not set");
    if (!m_impl->descriptors) throw std::runtime_error("RenderGraphBuilder: descriptors not set");
    if (!m_impl->window)      throw std::runtime_error("RenderGraphBuilder: window not set");
    if (m_impl->surface == VK_NULL_HANDLE)
        throw std::runtime_error("RenderGraphBuilder: surface not set");

    // Determine the swapchain color format from the surface.
    auto support  = Swapchain::QuerySupport(dev.PhysicalDevice(), m_impl->surface);
    VkFormat colorFmt = support.formats.empty()
        ? VK_FORMAT_B8G8R8A8_SRGB
        : support.formats[0].format;
    for (const auto& f : support.formats)
        if (f.format == VK_FORMAT_B8G8R8A8_SRGB) { colorFmt = f.format; break; }

    // Create the forward render pass first so its VkRenderPass handle is available
    // before the swapchain framebuffers are created.
    auto forwardPass = std::make_unique<ForwardRenderPass>();
    forwardPass->CreateRenderPass(dev, colorFmt, VK_FORMAT_D32_SFLOAT);

    // Create (or recreate) the swapchain using the new VkRenderPass.
    m_impl->swapchain->Recreate(dev, m_impl->surface, *m_impl->window,
                                 forwardPass->GetRenderPass());

    BuildPassInfo info{};
    info.dev              = &dev;
    info.forwardRenderPass = forwardPass->GetRenderPass();
    info.uboLayout        = m_impl->descriptors->Layout();
    info.extent           = m_impl->swapchain->Extent();
    info.framesInFlight   = RenderGraph::MAX_FRAMES;
    info.maxObjects       = m_impl->maxObjects;
    info.shaderDir        = m_impl->shaderDir;

    std::vector<std::unique_ptr<IPass>> passes;

    // Phase 3 (occlusion culling) passes would be added here.

    // Phase 2: frustum culling compute pass (runs before ForwardRenderPass).
    if (m_impl->options.frustumCulling)
        passes.push_back(std::make_unique<FrustumCullPass>());

    (void)m_impl->options.occlusionCulling;

    // ForwardRenderPass is always last.
    passes.push_back(std::move(forwardPass));

    RenderGraph graph;
    graph.Build(dev, *m_impl->swapchain, info, std::move(passes));
    return graph;
}

} // namespace xcel
